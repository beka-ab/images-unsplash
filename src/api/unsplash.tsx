...
const ACCESS_KEY = "2N7FmY6e7nQnFyHbKP8TDTI9ZwRgaUzFz4VvSJspxOk";
...
const getPopularPhotos = async (page: number): Promise<Photo[]> => {
  const response = await fetch(
    `${BASE_URL}/photos?page=${page}&per_page=20&order_by=popular&client_id=${ACCESS_KEY}`,
    {
      method: "GET",
    }
  );
...
const getSearchPhotos = async (
  ...
  const response = await fetch(
    `${BASE_URL}/search/photos?query=${query}&page=${page}&per_page=20&client_id=${ACCESS_KEY}`,
    {
      method: "GET",
    }
  );
